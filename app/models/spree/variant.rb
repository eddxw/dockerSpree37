...
module Spree
  class Variant < Spree::Base
    ...
    # searchkick
    searchkick word_start: %i[sku ean name]
    scope :search_import, -> { includes(:prices, :product) }
    def search_data
      {
        sku: sku,
        ean: ean,
    ...
    # scope :search_import, -> { includes(:prices, :product, :images) }
    # def search_data
    #   {
    #     sku: sku,
    #     ean: ean,
    #     price: price,
    #     name: product.name,
    #     image: Rails.application.routes.url_helpers.rails_representation_url(images.first.url(:mini), only_path: true)
    #   }
    # end
